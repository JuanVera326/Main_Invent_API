...
public interface IModDevRepository extends JpaRepository<ModDevVo, Long>{
	...
	@Query(nativeQuery = true, value = " SELECT * FROM moddev_items m WHERE tipo_moddev_item REGEXP CONCAT('^',?1)")
	...
	@Query(nativeQuery = true, value = "SELECT * from moddev_items WHERE id_moddev_item REGEXP CONCAT('^',?1)")
	public Iterable<ModDevVo> getId_moddev_items(Long id);
}
